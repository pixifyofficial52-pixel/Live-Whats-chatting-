...
function uploadProfilePicture() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            // Add crown to profile if user is special
...
function updateUserProfilePic(userId, profilePic) {
    ...
    if (userItem) {
        ...
        if (avatar) {
            ...
            // Check if user is special and add crown
...
function displaySearchResults(users) {
    ...
    users.forEach(user => {
        ...
        // Add crown icon if user is special
...
// ========== Add user to list with online/offline separation and crown ==========
function addUserToList(user) {
    ...
    const crownIcon = isSpecial ? '<i class="fas fa-crown crown-icon"></i>' : '';
...
function completeLogin() {
    ...
    // Update profile with name and check for special status
...
function selectUser(user) {
    ...
    // Update chat header with crown if user is special
...
function displayMessage(senderName, message, type, timestamp, messageId, messageData = {}) {
    ...
    // Add crown icon for received messages from special users
...
socket.on('all-users', (users) => {
    ...
    // Update current user's special status if present
...
socket.on('user-special-updated', (data) => {
    ...
    if (userItem) {
        ...
        const avatar = userItem.querySelector('.user-avatar');
        if (avatar) {
            const existingCrown = avatar.querySelector('.crown-badge');
            if (data.isSpecial) {
                if (!existingCrown) {
                    const crownDiv = document.createElement('div');
                    crownDiv.className = 'crown-badge';
        ...
        // Update name crown
        const h4 = userItem.querySelector('h4');
        if (h4) {
            const existingCrownIcon = h4.querySelector('.crown-icon');
            if (data.isSpecial) {
                if (!existingCrownIcon) {
                    const crownIcon = document.createElement('i');
                    crownIcon.className = 'fas fa-crown crown-icon';
                    h4.appendChild(crownIcon);
                }
            } else {
                if (existingCrownIcon) existingCrownIcon.remove();
            }
        }
    }
    
    // If this is current user, update profile
    ...
    // If this is selected user, update chat header
    ...
    showToast(`${data.name} is now ${data.isSpecial ? 'a special user 👑' : 'no longer special'}`, 'info');
});
    ...
    try {
        ...
        peerConnection.ontrack = (event) => {
            ...
            if (selectedUser?.userId !== pendingCall.fromUserId) {
                // Create temporary user object for call screen
                const tempUser = {
