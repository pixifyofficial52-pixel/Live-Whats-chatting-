...
// ========== CROWN PERSISTENCE SYSTEM ==========
const CROWN_CHECK_INTERVAL = 2000; // 2 seconds
let crownCheckTimer = null;
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            if (currentUser.isSpecial) {
                ...
                crownDiv.className = 'crown-badge-profile';
                crownDiv.innerHTML = '<i class="fas fa-crown"></i>';
                profilePicDiv.appendChild(crownDiv);
            }
...
function updateUserProfilePic(userId, profilePic) {
    ...
    if (userItem) {
        ...
        if (avatar) {
            avatar.innerHTML = `<img src="${profilePic}" alt="Profile">`;
            const user = allUsers.find(u => u.userId === userId);
            if (user && user.isSpecial) {
                const crownDiv = document.createElement('div');
                crownDiv.className = 'crown-badge';
                crownDiv.style.cssText = 'display: flex !important; position: absolute; bottom: -2px; right: -2px; background: #f1c40f; color: white; width: 18px; height: 18px; border-radius: 50%; align-items: center; justify-content: center; font-size: 10px; border: 2px solid white; z-index: 1000;';
                crownDiv.innerHTML = '<i class="fas fa-crown"></i>';
...
function displaySearchResults(users) {
    ...
    users.forEach(user => {
        ...
        const crownIcon = user.isSpecial ? '<i class="fas fa-crown" style="color: #f1c40f; margin-left: 5px;"></i>' : '';
...
function startChatWithUser(userId, userName, isSpecial) {
    if (!document.getElementById(`user-${userId}`)) {
        addUserToList({ userId, name: userName, profilePic: null, online: isUserOnline(userId), isSpecial: isSpecial });
    }
    selectUser({ userId, name: userName, profilePic: null, isSpecial: isSpecial });
    toggleGlobalSearch();
        ...
        callScreen.innerHTML = `
            <div class="call-container video">
                <div class="remote-video-container">
                    ...
                    <div class="call-info">
                        <h3>${selectedUser?.name || 'User'} ${selectedUser?.isSpecial ? '<i class="fas fa-crown" style="color: #f1c40f;"></i>' : ''}</h3>
...
// ========== CROWN MANAGEMENT SYSTEM ==========

// Force crown display function
function forceShowCrown() {
    console.log('👑 Force refreshing crowns for all users...');
    
    allUsers.forEach(user => {
        if (user.isSpecial) {
            const userDiv = document.getElementById(`user-${user.userId}`);
            if (userDiv) {
                // Avatar crown
                const avatar = userDiv.querySelector('.user-avatar');
                if (avatar) {
                    let crown = avatar.querySelector('.crown-badge');
                    if (!crown) {
                        crown = document.createElement('div');
                        crown.className = 'crown-badge';
                        crown.style.cssText = 'display: flex !important; position: absolute; bottom: -2px; right: -2px; background: #f1c40f; color: white; width: 18px; height: 18px; border-radius: 50%; align-items: center; justify-content: center; font-size: 10px; border: 2px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.2); z-index: 1000;';
                        crown.innerHTML = '<i class="fas fa-crown"></i>';
                        avatar.appendChild(crown);
                    } else {
                        crown.style.display = 'flex !important';
                    }
                }
                ...
                if (h4) {
                    let nameCrown = h4.querySelector('.crown-icon');
                    if (!nameCrown) {
                        nameCrown = document.createElement('i');
                        nameCrown.className = 'fas fa-crown crown-icon';
                        nameCrown.style.cssText = 'display: inline-block !important; color: #f1c40f !important; margin-left: 5px !important; font-size: 14px !important;';
                        h4.appendChild(nameCrown);
                    } else {
                        nameCrown.style.display = 'inline-block !important';
                    }
                }
            }
        }
    ...
    // Update current user if special
    if (currentUser && currentUser.isSpecial) {
        updateCurrentUserProfile();
    }
    
    // Update selected user if special
    if (selectedUser && selectedUser.isSpecial) {
        const crownIcon = '<i class="fas fa-crown chat-crown-badge" style="color:#f1c40f;"></i>';
        document.getElementById('chat-with-name').innerHTML = `${selectedUser.name} ${crownIcon}`;
    }
}

// Check crown status from server
async function checkCrownStatus() {
    if (!currentUser) return;
    
    try {
        const response = await fetch('/api/admin/special-users', {
            headers: {
                'admin-key': 'HJ-HACKER76768085&SBL-HACKER76768085'
            }
        });
        ...
        if (response.ok) {
            ...
            // Update allUsers array with latest special status
            allUsers.forEach(user => {
                const isSpecialNow = specialUsers.some(su => su.userId === user.userId);
                if (isSpecialNow !== user.isSpecial) {
                    console.log(`👑 Crown status changed for ${user.name}: ${user.isSpecial} -> ${isSpecialNow}`);
                    user.isSpecial = isSpecialNow;
                }
            });
            
            // Check if current user's status changed
            const currentUserSpecial = specialUsers.some(su => su.userId === currentUser.userId);
            if (currentUserSpecial !== currentUser.isSpecial) {
                console.log(`👑 Your crown status changed to: ${currentUserSpecial}`);
                currentUser.isSpecial = currentUserSpecial;
                updateCurrentUserProfile();
            }
        ...
        console.log('Crown check error:', error);
    }
}

// Start crown checker
function startCrownChecker() {
    if (crownCheckTimer) clearInterval(crownCheckTimer);
    crownCheckTimer = setInterval(checkCrownStatus, 2000);
    console.log('👑 Crown checker started');
}

// Add user to list with guaranteed crown display
function addUserToList(user) {
    ...
    const existing = document.getElementById(`user-${user.userId}`);
    ...
    // Force special status from allUsers if available
    let isSpecial = user.isSpecial === true;
    const allUserData = allUsers.find(u => u.userId === user.userId);
    if (allUserData && allUserData.isSpecial) {
        isSpecial = true;
    }
    ...
    // Crown HTML - ALWAYS SHOW IF SPECIAL
    const crownHtml = isSpecial 
        ? '<div class="crown-badge" style="display: flex !important; position: absolute; bottom: -2px; right: -2px; background: #f1c40f; color: white; width: 18px; height: 18px; border-radius: 50%; align-items: center; justify-content: center; font-size: 10px; border: 2px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.2); z-index: 1000;"><i class="fas fa-crown"></i></div>' 
        : '';
    
    const crownIcon = isSpecial 
        ? '<i class="fas fa-crown crown-icon" style="display: inline-block; color: #f1c40f; margin-left: 5px; font-size: 14px;"></i>' 
        : '';
...
function updateUserStatus(userId, isOnline) {
    ...
    statusEl.innerHTML = `<i class="fas fa-circle" style="color:${statusColor}; font-size:8px; margin-right:4px;"></i> ${statusText}`;
...
function isUserOnline(userId) {
    ...
    const statusEl = userDiv.querySelector('.user-info p');
    return statusEl.innerHTML.includes('#4caf50');
}
...
function completeLogin() {
    ...
    currentUser = { 
        userId: id, 
        name, 
        profilePic: loadUserData()?.profilePic || null,
        isSpecial: false 
    };
    saveUserData(currentUser);
    ...
    setTimeout(() => {
        socket.emit('get-all-users');
        startCrownChecker(); // Start crown checker after login
    }, 1000);
    
    // Initial crown check
    setTimeout(checkCrownStatus, 2000);
}
...
function logout() {
    if (confirm('Are you sure you want to logout?')) {
        ...
        const profilePicDiv = document.getElementById('profile-pic');
        profilePicDiv.innerHTML = '<i class="fas fa-camera" id="camera-icon"></i>';
        
        document.getElementById('messages-container').innerHTML = '';
        selectedUser = null;
        currentUser = null;
        document.getElementById('no-chat-selected').classList.remove('hidden');
        document.getElementById('chat-with-name').innerHTML = 'Select Contact';
    }
}
...
function selectUser(user) {
    ...
    document.getElementById('chat-with-status').innerHTML = '<i class="fas fa-circle" style="color:#4caf50; font-size:8px;"></i> Online';
...
function displayMessage(senderName, message, type, timestamp, messageId, messageData = {}) {
    ...
    div.innerHTML = `
        ${type === 'received' ? `<div class="sender">${senderCrown} ${senderName}</div>` : ''}
...
async function sendVoiceMessage() {
    ...
    mediaRecorder.onstop = async () => {
        ...
        displayVoiceMessage({ fromName: 'You', audioUrl: data.fileUrl, duration, timestamp: messageData.timestamp, messageId: messageId, fromUserSpecial: currentUser.isSpecial });
        saveMessageToHistory(selectedUser.userId, messageData);
...
function displayVoiceMessage(data) {
    ...
    if (data.deletedForEveryone) {
        div.innerHTML = `
            ${!isSent ? `<div class="sender">${senderCrown} ${data.fromName}</div>` : ''}
            <div class="deleted-message">
...
async function handleFileSelect(e) {
    ...
    displayFileMessage({ fromName: 'You', fileUrl: data.fileUrl, fileName: data.fileName, fileType: data.fileType, timestamp: messageData.timestamp, messageId: messageId, fromUserSpecial: currentUser.isSpecial });
    saveMessageToHistory(selectedUser.userId, messageData);
...
// ========== Update current user profile with crown ==========
function updateCurrentUserProfile() {
    if (!currentUser) return;
    
    document.getElementById('name-text').textContent = currentUser.name;
    document.getElementById('current-user-id').textContent = currentUser.userId;
    
    if (currentUser.isSpecial) {
        document.getElementById('profile-crown').style.display = 'flex';
        document.getElementById('name-crown').style.display = 'inline';
    } else {
        document.getElementById('profile-crown').style.display = 'none';
        document.getElementById('name-crown').style.display = 'none';
    }
    
    if (currentUser.profilePic) {
        const profilePicDiv = document.getElementById('profile-pic');
        profilePicDiv.innerHTML = `<img src="${currentUser.profilePic}" alt="Profile">`;
...
socket.on('all-users', (users) => {
    ...
    if (currentUserData && currentUser) {
        currentUser.isSpecial = currentUserData.isSpecial || false;
        currentUser.specialBadge = currentUserData.specialBadge || null;
        updateCurrentUserProfile();
    }
    ...
    // Force crown display after getting users
    setTimeout(forceShowCrown, 500);
});
...
socket.on('login-error', (message) => {
    ...
    localStorage.removeItem('hj-device-user-id');
    document.getElementById('login-name').value = '';
    document.getElementById('profile-pic').innerHTML = '<i class="fas fa-camera" id="camera-icon"></i>';
});
...
socket.on('user-online', (user) => {
    ...
    if (existingUser) {
        ...
        existingUser.isSpecial = user.isSpecial || false;
    } else {
...
// ========== UPDATED: Special user socket handler with force display ==========
socket.on('user-special-updated', (data) => {
    console.log('👑 Special user updated (FORCED):', data);
    
    // Update in allUsers array
    const userIndex = allUsers.findIndex(u => u.userId === data.userId);
    if (userIndex !== -1) {
        allUsers[userIndex].isSpecial = data.isSpecial;
    } else {
        // Add if not exists
        allUsers.push({
            userId: data.userId,
    ...
    // Force remove and re-add user to refresh display
    ...
    // Force show all crowns
    setTimeout(forceShowCrown, 100);
    
    showToast(`${data.name} is now ${data.isSpecial ? 'a special user 👑' : 'no longer special'}`, 'success');
});

socket.on('special-status-changed', (data) => {
    if (currentUser && data.userId === currentUser.userId) {
        currentUser.isSpecial = data.isSpecial;
        currentUser.specialBadge = data.badgeType;
        updateCurrentUserProfile();
        
        // Force crown for current user
        setTimeout(forceShowCrown, 100);
    }
});
...
socket.on('private-message', (data) => {
    ...
    if (selectedUser && selectedUser.userId === data.fromUserId) {
        displayMessage(data.fromName, data.message, 'received', timestamp, data.messageId, { 
            ...
            fromUserSpecial: data.fromUserSpecial
        });
...
socket.on('voice-message', (data) => {
    if (blockedUsers.includes(data.fromUserId)) return;
    if (selectedUser && selectedUser.userId === data.fromUserId) {
        displayVoiceMessage({ ...data, fromName: data.fromName, fromUserSpecial: data.fromUserSpecial });
        saveMessageToHistory(data.fromUserId, { 
            fromName: data.fromName, 
            type: 'voice', 
...
socket.on('file-message', (data) => {
    if (blockedUsers.includes(data.fromUserId)) return;
    if (selectedUser && selectedUser.userId === data.fromUserId) {
        displayFileMessage({ ...data, fromName: data.fromName, fromUserSpecial: data.fromUserSpecial });
        saveMessageToHistory(data.fromUserId, { 
            fromName: data.fromName, 
            type: 'file', 
            fileUrl: data.fileUrl, 
...
socket.on('call-offer', async (data) => {
    ...
    callDialog.innerHTML = `
        <div class="call-dialog-content">
            ...
            <p>${data.fromName} ${data.fromUserSpecial ? '<i class="fas fa-crown" style="color: #f1c40f;"></i>' : ''} is calling...</p>
...
window.acceptCall = async function() {
    ...
    try {
        ...
        peerConnection.ontrack = (event) => {
            ...
            const user = allUsers.find(u => u.userId === pendingCall.fromUserId);
            if (selectedUser?.userId !== pendingCall.fromUserId) {
                const tempUser = {
                    name: pendingCall.fromName,
                    userId: pendingCall.fromUserId,
                    isSpecial: user?.isSpecial || false
                };
                showCallScreenWithUser(pendingCall.callType, tempUser);
            } else {
...
function showCallScreenWithUser(type, user) {
    document.getElementById('call-dialog')?.remove();
        ...
        callScreen.innerHTML = `
            <div class="call-container video">
                <div class="remote-video-container">
                    <video id="remote-video" autoplay playsinline></video>
                    <div class="call-info">
                        <h3>${user.name} ${user.isSpecial ? '<i class="fas fa-crown" style="color: #f1c40f;"></i>' : ''}</h3>
                        <p class="call-timer" id="call-timer">00:00</p>
                    </div>
                </div>
                <div class="local-video-container">
...
window.addEventListener('load', async () => {
    ...
    if (userData && userData.userId === uniqueId) {
        ...
        if (userData.profilePic) {
            setTimeout(() => {
                const profilePicDiv = document.getElementById('profile-pic');
                profilePicDiv.innerHTML = `<img src="${userData.profilePic}" alt="Profile">`;
                if (currentUser.isSpecial) {
                    const crownDiv = document.createElement('div');
                    crownDiv.className = 'crown-badge-profile';
                    crownDiv.innerHTML = '<i class="fas fa-crown"></i>';
    ...
    // Start crown checker after everything loads
    setTimeout(startCrownChecker, 3000);
});
